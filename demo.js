...
seta();
alert(a);
注意没有块级作用域，变量查询中，访问局部变量要比全局变量更快，因为不需要向上搜索作用域链。
*/

/**
二.内存问题
JavaScript具有自动垃圾回收机制，也就说，执行环境会复杂管理代码执行过程中使用的内存。
JavaScript最常用的垃圾收集方式是标记清除，垃圾收集器会在运行说的时候给存储在内存中的变量加上标记
，然后去掉正在使用的变量的标记。没去掉的变量视为准备删除的变量。
垃圾收集器是周期性运行的。这样会导致程序性能问题，IE7以前版本，是根据内存分配量来运行的。256个变量运行
一次，有个概念解除引用：一旦数据不再使用，就将其设置为null来释放引用。这一做法适用于大多数全局
变量和全局对象
*/
